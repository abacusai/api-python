...
class CodeBot(AbstractApiClass):
    """
        ...
        Args:
            ...
            isCustomRouter (bool): Whether this entry is an org-defined custom router (custom_<slug>).
    """

    def __init__(self, client, llmName=None, name=None, coworkName=None, imageUploadSupported=None, codeAgentSupported=None, codeEditSupported=None, isPremium=None, llmBotIcon=None, provider=None, isUserApiKeyAllowed=None, isRateLimited=None, apiKeyUrl=None, isCustomRouter=None):
        ...
        self.is_custom_router = isCustomRouter
        self.deprecated_keys = {}

    def __repr__(self):
        repr_dict = {f'llm_name': repr(self.llm_name), f'name': repr(self.name), f'cowork_name': repr(self.cowork_name), f'image_upload_supported': repr(self.image_upload_supported), f'code_agent_supported': repr(self.code_agent_supported), f'code_edit_supported': repr(self.code_edit_supported), f'is_premium': repr(
            self.is_premium), f'llm_bot_icon': repr(self.llm_bot_icon), f'provider': repr(self.provider), f'is_user_api_key_allowed': repr(self.is_user_api_key_allowed), f'is_rate_limited': repr(self.is_rate_limited), f'api_key_url': repr(self.api_key_url), f'is_custom_router': repr(self.is_custom_router)}
        class_name = "CodeBot"
    ...
    def to_dict(self):
        ...
        resp = {'llm_name': self.llm_name, 'name': self.name, 'cowork_name': self.cowork_name, 'image_upload_supported': self.image_upload_supported, 'code_agent_supported': self.code_agent_supported, 'code_edit_supported': self.code_edit_supported,
                'is_premium': self.is_premium, 'llm_bot_icon': self.llm_bot_icon, 'provider': self.provider, 'is_user_api_key_allowed': self.is_user_api_key_allowed, 'is_rate_limited': self.is_rate_limited, 'api_key_url': self.api_key_url, 'is_custom_router': self.is_custom_router}
