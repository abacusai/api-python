...
class CoworkDispatchMessage(AbstractApiClass):
    ...
    def __init__(self, client, dispatchMessageId=None, state=None, deploymentConversationId=None, deliverySeq=None, targetDesktopDeviceId=None, mobileDeviceId=None, messageText=None, workspaceOverride=None, createdAt=None, ackedAt=None, completedAt=None):
    ...
    def __repr__(self):
        repr_dict = {f'dispatch_message_id': repr(self.dispatch_message_id), f'state': repr(self.state), f'deployment_conversation_id': repr(self.deployment_conversation_id), f'delivery_seq': repr(self.delivery_seq), f'target_desktop_device_id': repr(
            self.target_desktop_device_id), f'mobile_device_id': repr(self.mobile_device_id), f'message_text': repr(self.message_text), f'workspace_override': repr(self.workspace_override), f'created_at': repr(self.created_at), f'acked_at': repr(self.acked_at), f'completed_at': repr(self.completed_at)}
    ...
    def to_dict(self):
        ...
        resp = {'dispatch_message_id': self.dispatch_message_id, 'state': self.state, 'deployment_conversation_id': self.deployment_conversation_id, 'delivery_seq': self.delivery_seq, 'target_desktop_device_id': self.target_desktop_device_id,
                'mobile_device_id': self.mobile_device_id, 'message_text': self.message_text, 'workspace_override': self.workspace_override, 'created_at': self.created_at, 'acked_at': self.acked_at, 'completed_at': self.completed_at}
