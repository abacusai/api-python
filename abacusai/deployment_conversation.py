...
class DeploymentConversation(AbstractApiClass):
    """
        ...
        Args:
            ...
            predictionOverrides (dict): Per-conversation prediction overrides; carries the last-used virtual ChatLLM model's llm_name so the picker/URL can restore it on reopen.
            ...
            firstVisibleEventIndex (int): The conversation_sequence_number of the first visible event, for FE pagination boundary.
            ...
            sharedHomeDirectory (bool): Whether this conversation is part of a shared-home-directory group — true if it has a parent conversation or at least one child conversation.
            ...
            agentType (str): The type of agent for the deployment conversation (e.g., MASTER_AGENT).
            agentLifecycle (str): The lifecycle state of the agent (PENDING, STOPPED, COMPLETED).
            orchestratorDeploymentConversationId (str): The deployment conversation id of the orchestrating master agent, if this conversation is a worker in an agent swarm.
            projectCreditsUsed (int): The total credits used by all the conversations in the project.
            webAppProjectId (str): The web app project id associated with the deployment conversation, if any.
            personalAgentComputerId (str): For SuperComputer conversations, the personal agent computer (VM) this conversation is pinned to, if any.
            roleplayCharacter (dict): For Role Play conversations, the minimal character display (name and image) attached so a viewer who does not own the private character can still see who they're chatting with.
    ...
    def __init__(self, client, deploymentConversationId=None, name=None, deploymentId=None, ownerUserId=None, ownerOrgId=None, createdAt=None, lastEventCreatedAt=None, hasHistory=None, externalSessionId=None, regenerateAttempt=None, externalApplicationId=None, unusedDocumentUploadIds=None, humanizeInstructions=None, conversationWarning=None, conversationType=None, metadata=None, predictionOverrides=None, llmDisplayName=None, llmBotIcon=None, searchSuggestions=None, chatllmTaskId=None, chatllmProjectId=None, conversationStatus=None, computerStatus=None, filesystemStatus=None, totalEvents=None, firstVisibleEventIndex=None, contestNames=None, daemonTaskId=None, parentDeploymentConversationId=None, sharedHomeDirectory=None, introMessage=None, previewInfo=None, latestContext=None, shareType=None, pushNotificationsEnabled=None, platform=None, agentType=None, agentLifecycle=None, orchestratorDeploymentConversationId=None, projectCreditsUsed=None, webAppProjectId=None, personalAgentComputerId=None, roleplayCharacter=None, history={}, hostedArtifacts={}, mobileBuildInfo={}):
        ...
        self.first_visible_event_index = firstVisibleEventIndex
        ...
        self.shared_home_directory = sharedHomeDirectory
        ...
        self.mobile_build_info = client._build_class(
            MobileBuildInfo, mobileBuildInfo)
        self.deprecated_keys = {}

    def __repr__(self):
        repr_dict = {f'deployment_conversation_id': repr(self.deployment_conversation_id), f'name': repr(self.name), f'deployment_id': repr(self.deployment_id), f'owner_user_id': repr(self.owner_user_id), f'owner_org_id': repr(self.owner_org_id), f'created_at': repr(self.created_at), f'last_event_created_at': repr(self.last_event_created_at), f'has_history': repr(self.has_history), f'external_session_id': repr(self.external_session_id), f'regenerate_attempt': repr(self.regenerate_attempt), f'external_application_id': repr(self.external_application_id), f'unused_document_upload_ids': repr(self.unused_document_upload_ids), f'humanize_instructions': repr(self.humanize_instructions), f'conversation_warning': repr(self.conversation_warning), f'conversation_type': repr(self.conversation_type), f'metadata': repr(self.metadata), f'prediction_overrides': repr(self.prediction_overrides), f'llm_display_name': repr(self.llm_display_name), f'llm_bot_icon': repr(self.llm_bot_icon), f'search_suggestions': repr(self.search_suggestions), f'chatllm_task_id': repr(self.chatllm_task_id), f'chatllm_project_id': repr(self.chatllm_project_id), f'conversation_status': repr(self.conversation_status), f'computer_status': repr(
            self.computer_status), f'filesystem_status': repr(self.filesystem_status), f'total_events': repr(self.total_events), f'first_visible_event_index': repr(self.first_visible_event_index), f'contest_names': repr(self.contest_names), f'daemon_task_id': repr(self.daemon_task_id), f'parent_deployment_conversation_id': repr(self.parent_deployment_conversation_id), f'shared_home_directory': repr(self.shared_home_directory), f'intro_message': repr(self.intro_message), f'preview_info': repr(self.preview_info), f'latest_context': repr(self.latest_context), f'share_type': repr(self.share_type), f'push_notifications_enabled': repr(self.push_notifications_enabled), f'platform': repr(self.platform), f'agent_type': repr(self.agent_type), f'agent_lifecycle': repr(self.agent_lifecycle), f'orchestrator_deployment_conversation_id': repr(self.orchestrator_deployment_conversation_id), f'project_credits_used': repr(self.project_credits_used), f'web_app_project_id': repr(self.web_app_project_id), f'personal_agent_computer_id': repr(self.personal_agent_computer_id), f'roleplay_character': repr(self.roleplay_character), f'history': repr(self.history), f'hosted_artifacts': repr(self.hosted_artifacts), f'mobile_build_info': repr(self.mobile_build_info)}
    ...
    def to_dict(self):
        ...
        """
        resp = {'deployment_conversation_id': self.deployment_conversation_id, 'name': self.name, 'deployment_id': self.deployment_id, 'owner_user_id': self.owner_user_id, 'owner_org_id': self.owner_org_id, 'created_at': self.created_at, 'last_event_created_at': self.last_event_created_at, 'has_history': self.has_history, 'external_session_id': self.external_session_id, 'regenerate_attempt': self.regenerate_attempt, 'external_application_id': self.external_application_id, 'unused_document_upload_ids': self.unused_document_upload_ids, 'humanize_instructions': self.humanize_instructions, 'conversation_warning': self.conversation_warning, 'conversation_type': self.conversation_type, 'metadata': self.metadata, 'prediction_overrides': self.prediction_overrides, 'llm_display_name': self.llm_display_name, 'llm_bot_icon': self.llm_bot_icon, 'search_suggestions': self.search_suggestions, 'chatllm_task_id': self.chatllm_task_id, 'chatllm_project_id': self.chatllm_project_id, 'conversation_status': self.conversation_status, 'computer_status': self.computer_status, 'filesystem_status': self.filesystem_status,
                'total_events': self.total_events, 'first_visible_event_index': self.first_visible_event_index, 'contest_names': self.contest_names, 'daemon_task_id': self.daemon_task_id, 'parent_deployment_conversation_id': self.parent_deployment_conversation_id, 'shared_home_directory': self.shared_home_directory, 'intro_message': self.intro_message, 'preview_info': self.preview_info, 'latest_context': self.latest_context, 'share_type': self.share_type, 'push_notifications_enabled': self.push_notifications_enabled, 'platform': self.platform, 'agent_type': self.agent_type, 'agent_lifecycle': self.agent_lifecycle, 'orchestrator_deployment_conversation_id': self.orchestrator_deployment_conversation_id, 'project_credits_used': self.project_credits_used, 'web_app_project_id': self.web_app_project_id, 'personal_agent_computer_id': self.personal_agent_computer_id, 'roleplay_character': self.roleplay_character, 'history': self._get_attribute_as_dict(self.history), 'hosted_artifacts': self._get_attribute_as_dict(self.hosted_artifacts), 'mobile_build_info': self._get_attribute_as_dict(self.mobile_build_info)}
        return {key: value for key, value in resp.items() if value is not None and key not in self.deprecated_keys}

    def get_org_connector_auth(self):
        """
        ...
        """
        return self.client.get_org_connector_auth(self.deployment_conversation_id)
