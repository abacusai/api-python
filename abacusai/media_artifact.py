...
class MediaArtifact(AbstractApiClass):
    ...
    def __init__(self, client, mediaArtifactId=None, mediaType=None, info=None, sourceDeploymentConversationId=None, origin=None, docId=None, isFavorited=None, isShared=None, publicUrl=None, thumbnailUrl=None, mediaUrl=None, filename=None, createdAt=None, updatedAt=None):
    ...
    def __repr__(self):
        repr_dict = {f'media_artifact_id': repr(self.media_artifact_id), f'media_type': repr(self.media_type), f'info': repr(self.info), f'source_deployment_conversation_id': repr(self.source_deployment_conversation_id), f'origin': repr(self.origin), f'doc_id': repr(self.doc_id), f'is_favorited': repr(
    ...
    def to_dict(self):
        ...
        """
        resp = {'media_artifact_id': self.media_artifact_id, 'media_type': self.media_type, 'info': self.info, 'source_deployment_conversation_id': self.source_deployment_conversation_id, 'origin': self.origin, 'doc_id': self.doc_id,
                'is_favorited': self.is_favorited, 'is_shared': self.is_shared, 'public_url': self.public_url, 'thumbnail_url': self.thumbnail_url, 'media_url': self.media_url, 'filename': self.filename, 'created_at': self.created_at, 'updated_at': self.updated_at}
