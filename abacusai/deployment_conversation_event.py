...
class DeploymentConversationEvent(AbstractApiClass):
    """
        ...
        Args:
            ...
            voiceoverCreditsUsed (float): The cumulative credits used generating this message's voiceover audio.
    ...
    def __init__(self, client, role=None, text=None, timestamp=None, messageIndex=None, regenerateAttempt=None, modelVersion=None, searchResults=None, isUseful=None, feedback=None, feedbackType=None, docInfos=None, keywordArguments=None, inputParams=None, attachments=None, responseVersion=None, agentWorkflowNodeId=None, nextAgentWorkflowNodeId=None, chatType=None, agentResponse=None, error=None, segments=None, streamedData=None, streamedSectionData=None, highlights=None, llmDisplayName=None, deepagentMode=None, llmBotIcon=None, formResponse=None, routedLlm=None, computePointsUsed=None, voiceoverCreditsUsed=None, computerFiles=None, toolUseRequest=None, toolUseResult=None, verificationSummary=None, attachedUserFileNames=None, parallelToolResponses=None, oldFileContent=None, allVersions=None, selectedVersionIndex=None, totalVersions=None, isLastMessage=None, allowTaskAutomation=None, mediaStudioContext=None):
        ...
        self.voiceover_credits_used = voiceoverCreditsUsed
    ...
    def __repr__(self):
        repr_dict = {f'role': repr(self.role), f'text': repr(self.text), f'timestamp': repr(self.timestamp), f'message_index': repr(self.message_index), f'regenerate_attempt': repr(self.regenerate_attempt), f'model_version': repr(self.model_version), f'search_results': repr(self.search_results), f'is_useful': repr(self.is_useful), f'feedback': repr(self.feedback), f'feedback_type': repr(self.feedback_type), f'doc_infos': repr(self.doc_infos), f'keyword_arguments': repr(self.keyword_arguments), f'input_params': repr(self.input_params), f'attachments': repr(self.attachments), f'response_version': repr(self.response_version), f'agent_workflow_node_id': repr(self.agent_workflow_node_id), f'next_agent_workflow_node_id': repr(self.next_agent_workflow_node_id), f'chat_type': repr(self.chat_type), f'agent_response': repr(self.agent_response), f'error': repr(self.error), f'segments': repr(self.segments), f'streamed_data': repr(self.streamed_data), f'streamed_section_data': repr(self.streamed_section_data), f'highlights': repr(
            self.highlights), f'llm_display_name': repr(self.llm_display_name), f'deepagent_mode': repr(self.deepagent_mode), f'llm_bot_icon': repr(self.llm_bot_icon), f'form_response': repr(self.form_response), f'routed_llm': repr(self.routed_llm), f'compute_points_used': repr(self.compute_points_used), f'voiceover_credits_used': repr(self.voiceover_credits_used), f'computer_files': repr(self.computer_files), f'tool_use_request': repr(self.tool_use_request), f'tool_use_result': repr(self.tool_use_result), f'verification_summary': repr(self.verification_summary), f'attached_user_file_names': repr(self.attached_user_file_names), f'parallel_tool_responses': repr(self.parallel_tool_responses), f'old_file_content': repr(self.old_file_content), f'all_versions': repr(self.all_versions), f'selected_version_index': repr(self.selected_version_index), f'total_versions': repr(self.total_versions), f'is_last_message': repr(self.is_last_message), f'allow_task_automation': repr(self.allow_task_automation), f'media_studio_context': repr(self.media_studio_context)}
    ...
    def to_dict(self):
        ...
        """
        resp = {'role': self.role, 'text': self.text, 'timestamp': self.timestamp, 'message_index': self.message_index, 'regenerate_attempt': self.regenerate_attempt, 'model_version': self.model_version, 'search_results': self.search_results, 'is_useful': self.is_useful, 'feedback': self.feedback, 'feedback_type': self.feedback_type, 'doc_infos': self.doc_infos, 'keyword_arguments': self.keyword_arguments, 'input_params': self.input_params, 'attachments': self.attachments, 'response_version': self.response_version, 'agent_workflow_node_id': self.agent_workflow_node_id, 'next_agent_workflow_node_id': self.next_agent_workflow_node_id, 'chat_type': self.chat_type, 'agent_response': self.agent_response, 'error': self.error, 'segments': self.segments, 'streamed_data': self.streamed_data, 'streamed_section_data': self.streamed_section_data, 'highlights': self.highlights,
                'llm_display_name': self.llm_display_name, 'deepagent_mode': self.deepagent_mode, 'llm_bot_icon': self.llm_bot_icon, 'form_response': self.form_response, 'routed_llm': self.routed_llm, 'compute_points_used': self.compute_points_used, 'voiceover_credits_used': self.voiceover_credits_used, 'computer_files': self.computer_files, 'tool_use_request': self.tool_use_request, 'tool_use_result': self.tool_use_result, 'verification_summary': self.verification_summary, 'attached_user_file_names': self.attached_user_file_names, 'parallel_tool_responses': self.parallel_tool_responses, 'old_file_content': self.old_file_content, 'all_versions': self.all_versions, 'selected_version_index': self.selected_version_index, 'total_versions': self.total_versions, 'is_last_message': self.is_last_message, 'allow_task_automation': self.allow_task_automation, 'media_studio_context': self.media_studio_context}
